...
export default class DropRenderIjazah extends Component {
    constructor(props){
        super(props)
        this.state={
            karyaUpload:null,
    ...
    componentDidMount(){
        window.onload = function() {
            if (performance.navigation.type === 1) {
                localStorage.removeItem('ijazahoff');
            }
        }
    }
    handleOnDrop = (files,reject) =>{
       ...
       if(files && files.length > 0){
        ...
        localStorage.setItem('ijazahoff',this.state.preview)
  ...
  render() {
    ...
    const prevOffline = localStorage.getItem("ijazahoff")
    return <div>
        {
            ...
            </div> : <center> <Dropzone {...input} onDrop={this.handleOnDrop} maxSize={maxSize} multiple={false} accept=".zip"><center>{label}</center>
            <br/>
